...
class MobilePSIItem extends AbstractPSIItem {
  constructor(
    url, 
    title, 
    status, 
  ...
  static loadFromPSIResponse(response) {
    ...
    let usabilityScore = -1;
    if (response.ruleGroups["USABILITY"]) {
      usabilityScore = response.ruleGroups["USABILITY"].score;
    }

    let item = new MobilePSIItem(
      response.id,
      response.title,
    ...
    //TODO: Add usability rule issues?
    return item;
  }
}

module.exports = MobilePSIItem;