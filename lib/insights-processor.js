...
var getPSI = limit(function(url, psiOpt, callback) {
  INSIGHTS_LOGGER.info(`Fetching ${url} for ${psiOpt.strategy}`);
  ...
  .then((res) => { //If succeeded, then call our callback
    ...
    INSIGHTS_LOGGER.error(`Err Fetching ${url} for ${psiOpt.strategy}. ${err}`);
    callback(err);
  });
}).to(1).per(1500);
...
class InsightsProcessor {
  ...
  _getInsightsAndSave(url) {
    ...
    return this._createContentFolder()
      ...
      // Get Desktop version
      .then(() => {
        return retry(this._getInsights.bind(this, url, 'desktop'), { interval: 60000 });
      })
      // Save Desktop
      .then((results) => {
      ...
      // Get Mobile versions
      .then(() => {
        return retry(this._getInsights.bind(this, url, 'mobile'), { interval: 60000 });
      })
      // Save Mobile
      .then((results) => {
  ...
  process() {
    ...
    let rtnPromise =
      ...
      fsp.mkdirAsync(this.outputFolder)
