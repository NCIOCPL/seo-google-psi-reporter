...
class InsightsProcessor {
  ...
  _getStrategyAndSave(url, strategy) {
    ...
    let promiseChain = new Promise((resolve, reject) => {
      ...
      // Get Desktop version
      return retry(this._getInsights.bind(this, url, strategy), { interval: this.psi_waittime_between_errors, max_tries: this.psi_max_retries })
    ...
    .then(
      (results) => {
      this.logger.debug(`${strategy}: ${results.id} : ${results.ruleGroups.SPEED.score}`);
  ...
  _getInsightsAndSave(url) {
    ...
    //This should be an array of promises.
    let promiseChain = new Promise((resolve, reject) => {
        this.logger.debug(`Fetching Desktop: ${url}`);
        // Get Desktop version
