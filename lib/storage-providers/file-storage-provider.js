...
class FileStorageProvider extends AbstractStorageProvider {
     ...
     */
    onBegin(urlList) {
        logger.debug("FileStorageProvider: Initializing Run");
        //Create folder
    ...
    storePSIRecord(url, strategy, psiRecord) {
        logger.debug(`FileStorageProvider: Saving ${url} for ${strategy}`);
        return Promise.resolve();
    }
    ...
    onEnd(totalUrls, itemsProcessed, itemsFetched, errorCount) {
        logger.debug(`FileStorageProvider: Ending Run`);
    ...
    onFatalError(message) {
        logger.debug(`FileStorageProvider: Handling Fatal Errors`);
